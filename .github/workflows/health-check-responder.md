---
description: |
  Reacts to weekly health check reports for the fluent-bit SUSE rebuild
...
on:
  workflow_run:
    workflows: ["Weekly Health Check — Fluent Bit (SUSE Rebuild)"]
    types: [completed]
...
safe-outputs:
  dispatch-workflow:
    workflows: ["auto-update-bci", "cve-response.lock"]
  add-comment:
    max: 10
